...
def _keypoints_render(items) -> str:
    ...
    rubric in rubrics.py is a "- " bulleted list. Rendering key points with "- " too made
    the two read as one list: the judge counted the five rubric criteria as points 1-5 and
    the real key points as 6-10, then rejected a perfectly grounded set for "invented
    points (4, 5, 6, 7, 8)" -- items 4-5 being its own criteria.

    Numbering makes the content unmistakably a separate list, which is why the other
    list-shaped tasks (mcq, practice_qsn) never showed the fault: both already render as
    "Q1.", "Q2.".
    """
    return "\n".join(f"{i}. {point}"
