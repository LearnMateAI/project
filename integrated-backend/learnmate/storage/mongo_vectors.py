...
class MongoVectorStore(VectorStore):
    ...
    def ping(self) -> Dict[str, Any]:
        """
        Is the store reachable, and how many chunks does it hold?

        The same shape QdrantVectorStore.ping returns, so a health check does not have to
        know which backend it is talking to. There is no collection to create here -- the
        chunks live in the same database as everything else -- so it is always "exists".
        """
        return {
            "backend": "mongodb",
