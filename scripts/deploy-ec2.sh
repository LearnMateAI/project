...
wait_for_keycloak_http() {
  local timeout="${1:-300}"
  local elapsed=0
  echo "Waiting for Keycloak to become ready (up to ${timeout}s)..."
  while [ "$elapsed" -lt "$timeout" ]; do
    if $COMPOSE_CMD exec -T keycloak sh -c 'exec 3<> /dev/tcp/127.0.0.1/8080' >/dev/null 2>&1; then
      echo "Keycloak is ready (port 8080 responding)"
      return 0
    fi
    sleep 5
    elapsed=$((elapsed + 5))
  done
  echo "Keycloak did not become ready within ${timeout}s" >&2
